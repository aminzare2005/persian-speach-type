...
export type ShenavaModelSource = "local" | "nimruz" | null;

export type ShenavaModelStatus = {
  modelKey: ShenavaModelKey;
  phase: ShenavaModelPhase;
  installed: boolean;
  /** Where a ready install was found. Local app root wins over Nimruz. */
  source: ShenavaModelSource;
  /**
   * Resolved install directory when present; otherwise the expected local
   * download destination. Empty until main process status loads.
   */
  path: string;
