...
export const PST_CODEX_PERMISSION_PROFILE = "pst-correction";

export function createManagedCodexConfig() {
  return `# Managed by Persian Speach Type. This Codex home is isolated from the Codex CLI.
approval_policy = "never"
default_permissions = "${PST_CODEX_PERMISSION_PROFILE}"
web_search = "disabled"
check_for_update_on_startup = false
include_apps_instructions = false
include_collaboration_mode_instructions = false

[permissions.${PST_CODEX_PERMISSION_PROFILE}]
description = "Read-only access to Persian Speach Type's dedicated empty workspace."

[permissions.${PST_CODEX_PERMISSION_PROFILE}.filesystem]
":minimal" = "read"

[permissions.${PST_CODEX_PERMISSION_PROFILE}.filesystem.":workspace_roots"]
"." = "read"

[permissions.${PST_CODEX_PERMISSION_PROFILE}.network]
enabled = false
...
export class CodexAppServerClient {
  ...
  private async startInner() {
    ...
    try {
      ...
      throw error instanceof Error
        ? error
        : new Error("اجرای موتور Codex ممکن نشد.");
    }
    this.process = child;
    ...
    child.once("exit", (code, signal) => {
      ...
      this.handleExit(child, new Error(`Codex به‌طور غیرمنتظره متوقف شد${suffix}.`));
    });
    ...
    try {
      await this.sendRequest(
        ...
        {
          clientInfo: {
            name: "persian_speach_type_desktop",
  ...
  private sendRequest<T>(method: string, params: unknown, timeoutMs: number) {
    ...
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`درخواست Codex تمام شد: ${method}`));
      }, timeoutMs);
      this.pending.set(id, {
        method,
      ...
      try {
        ...
        this.pending.delete(id);
        reject(
          error instanceof Error ? error : new Error("ارتباط با Codex برقرار نشد.")
        );
      }
    });
  }
  ...
  private send(message: RpcMessage) {
    if (!this.process?.stdin.writable) {
      throw new Error("Codex در حال اجرا نیست.");
    }
  ...
  private handleLine(line: string) {
    ...
    if (message.method && message.id !== undefined) {
      this.send({
        ...
        error: {
          code: -32601,
          message: `Persian Speach Type does not support server request ${message.method}.`,
        },
      });
      return;
    }
  ...
  dispose() {
    ...
    const error = new Error("Codex متوقف شد.");
    for (const pending of this.pending.values()) {
      clearTimeout(pending.timer);
      pending.reject(error);
    }
