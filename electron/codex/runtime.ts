...
export function resolveCodexExecutable(options?: {
  ...
  try {
    ...
    throw new Error("موتور Codex نصب نشده است. وابستگی‌ها را دوباره نصب کنید.");
  }
  ...
  try {
    ...
    throw new Error(
      `موتور Codex برای ${platform}/${arch} پیدا نشد. وابستگی‌ها را دوباره نصب کنید.`
    );
  }
  ...
  if (!executable) {
    throw new Error(
      `فایل اجرایی Codex برای ${platform}/${arch} به‌درستی بسته‌بندی نشده است.`
    );
  }
  return executable;
}
