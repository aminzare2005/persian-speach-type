...
type ModelRoot = {
  path: string;
  source: Exclude<ShenavaModelSource, null>;
};
...
export class ShenavaService {
  ...
  constructor(options: {
    userDataPath: string;
    workerScript: string;
    /** Extra model roots (e.g. Nimruz). Local app root is always first. */
    sharedModelRoots?: string[];
  }) {
    this.#modelsRoot = path.join(options.userDataPath, "models");
    this.#modelRoots = [
      { path: this.#modelsRoot, source: "local" },
      ...(options.sharedModelRoots ?? []).map(
        (root): ModelRoot => ({ path: root, source: "nimruz" })
      ),
    ];
  ...
  /** Local install / download target directory (app userData only). */
  getModelDirectory(modelKey: ShenavaModelKey) {
    return path.join(this.#modelsRoot, SHENAVA_MODELS[modelKey].directoryName);
  }

  /** True when `targetPath` is under local or known shared (Nimruz) model roots. */
  isAllowedModelPath(targetPath: string) {
    const resolved = path.resolve(targetPath);
    return this.#modelRoots.some((root) => {
  ...
  /** First valid install across local + shared (Nimruz) roots. */
  async resolveModelDirectory(
    modelKey: ShenavaModelKey
  ): Promise<{ directory: string; source: Exclude<ShenavaModelSource, null> } | null> {
    for (const root of this.#modelRoots) {
      const directory = path.join(
        root.path,
        SHENAVA_MODELS[modelKey].directoryName
      );
      if (await this.#isValidInstall(modelKey, directory)) {
        return { directory, source: root.source };
      }
    }
    return null;
  }
  ...
  async remove(modelKeyInput: ShenavaModelKey): Promise<ShenavaStatus> {
    ...
    // Never delete Nimruz-owned files — only the local app copy.
  ...
  async transcribe(samples: Float32Array): Promise<ShenavaTranscription> {
    ...
    const resolved = await this.resolveModelDirectory(modelKey);
    if (!resolved) {
      throw new Error("The active Shenava model is not installed.");
    }

    return this.#runWorker(resolved.directory, samples);
  }
  ...
  #baseStatus(modelKey: ShenavaModelKey): ShenavaModelStatus {
    return {
      ...createInitialShenavaModelStatus(modelKey),
      path: this.getModelDirectory(modelKey),
    };
  }
  ...
  async #readModelStatus(
    ...
    if (resolved) {
      try {
        const [modelStats, tokenStats] = await Promise.all([
          stat(path.join(resolved.directory, "model.onnx")),
          stat(path.join(resolved.directory, "tokens.txt")),
        ]);
        return {
          ...this.#baseStatus(modelKey),
          phase: "ready",
          installed: true,
          source: resolved.source,
          path: resolved.directory,
          downloadedBytes: model.totalBytes,
          installedBytes: modelStats.size + tokenStats.size,
        };
      } catch {
        // Fall through to partial/local download progress.
      }
    }
  ...
  async #isValidInstall(modelKey: ShenavaModelKey, directory: string) {
    const model = SHENAVA_MODELS[modelKey];
    try {
      const [manifest, modelStats, tokenStats] = await Promise.all([
        this.#readManifestAt(directory),
  ...
  async #performDownload(
    ...
    const localPath = this.getModelDirectory(modelKey);
    ...
    try {
      ...
      await rename(temporaryDirectory, localPath);
      await this.#writeSelection(modelKey);
  ...
  async #readManifestAt(directory: string): Promise<InstallManifest> {
